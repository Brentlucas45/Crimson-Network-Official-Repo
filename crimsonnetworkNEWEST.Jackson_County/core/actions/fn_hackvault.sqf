...
if(_cops < 0) exitWith { hint "You need atleast 5 cops online to rob the bank..."; }; 
...
if (life_inv_hackingtool > 0 && !hacking && cgbankvault animationPhase "d_l_Anim" == 0 ) then {
	...
	while {_num > 0} do {
		if ( _num == 300 || _num == 240 || _num == 180 || _num == 120 || _num == 60 ) then {
			...
			playSound3D ["dag_soimtx\sounds\starthack.ogg", player, false, getPosASL player, 1, 1, 225];
	...
	if(alive player) then {
		...
		cgbankvault setVariable["hacking",false,true];
		playSound3D ["dag_soimtx\sounds\endhack.ogg", player, false, getPosASL player, 1, 1, 225];
